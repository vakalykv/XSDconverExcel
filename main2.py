...
def parse_xml_schema(file_path):
    ...
    def extract_elements(element, xpath=""):
        ...
        # annotation_elem = element.find('xs:annotation/xs:documentation', namespaces)
        ...
        if name:
            ...
            #extract_elements(choice, element_xpath)
...
def create_excel(elements, output_file):
    ...
    headers = ["Номер по порядку", "Название элемента", "Описание элемента", "Признак обязательности", "Повторяемость",
               "XPath", "Пояснение MDR", "Пояснение НРД", "Тип данных", "Значения Enumeration"]
...
if __name__ == "__main__":
    ...
    #print(f"Массив elements  {elements} основная программа")
    # Создаем Excel файл
